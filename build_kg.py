...
class GoodsKg:
    def __init__(self):
        ...
        self.g = Graph(
            host="47.106.139.236",  # neo4j 搭载服务器的ip地址，ifconfig可获取到 neo4j://47.106.139.236:7687
            http_port=9474,  # neo4j 服务器监听的端口号
    ...
    def create_edges(self, rels, start_type, end_type):
        ...
        for rel in set(rels):
            ...
            # 'match (m:Brand), (n:Product) where m.name = "飞利浦（PHILIPS）" and n.name = "智能车机" create (m)-[:sales{name:"销售"}]->(n)'
            try:
                self.g.run(sql)
